...
export const WorkspaceLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useWorkspaceQuery(queryPage);
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row?.original?.country}</Text>
          </div>
        );
      },
    },
    {
        ...
        cell: ({ row }: any) => {
          return (
            <div className="flex items-center gap-2 font-medium text-text-secondary">
              <Text>{row?.original?.country_code}</Text>
            </div>
          );
        },
      },
      ...
      cell: ({ row }: any) => (
        <div className="flex items-center gap-3">
          <EyeIcon
            ...
            onClick={() => {
              // router.push(`/app/workspace/${row?.original?._id}/business`);
              router.push(`/app/workspace/${row?.original?._id}/workspace-overview`);
            }}
          />
        </div>
      ),

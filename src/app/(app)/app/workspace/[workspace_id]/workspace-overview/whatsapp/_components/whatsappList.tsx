...
export const WhatsappList = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useWorkspaceByIdQuery(workspaceId);
  ...
  useEffect(() => {
    if (data) {
      ...
      setItems(businessList as never[]);

      setPagination({
        per_page: data.per_page,
        total_page: 1,
        total_result: businessList.length,
        current_page: 1,
      });

      setQueryPage((prevState: any) => ({
        ...prevState,
        per_page: businessList.length,
        page: 1,
      }));
    }
  }, [data]);
      ...
      cell: ({ row }: any) => (
        <div className="flex items-center gap-3">
          <EyeIcon
            ...
            onClick={() => {
              router.push(`/app/workspace/${workspaceId}/workspace-overview/whatsapp/business/${row?.original?._id}/overview`);
            }}
          />
        </div>
      ),

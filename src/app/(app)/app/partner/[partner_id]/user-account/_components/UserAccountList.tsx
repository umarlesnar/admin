...
import { useOnboardingUseraccount } from "@/framework/partner/user_account/get-useraccount";
import { useOboardingUserAccountMutation } from "@/framework/partner/user_account/useraccount-mutation";
...
export const UserAccountLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useOnboardingUseraccount(queryPage);
  const { mutateAsync } = useOboardingUserAccountMutation();
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="flex items-center gap-4">
            <EditUserAccountSheet data={userAccount}>
              <EditIcon className="w-4 h-4 cursor-pointer text-green-600" />
            </EditUserAccountSheet>
            <>
              <Alert
                icon={<AlertIcon className="w-[50px] h-[50px] text-red-500" />}
                description={`Do you want to remove this ${row?.original?.profile?.first_name} useraccount ?`}
                onRightButton={async () => {
                  const loadingToast = toast.loading("Loading...");
                  try {
                    await mutateAsync({
                      method: "DELETE",
                      user_account_id: row?.original?._id,
                    });

...
export const AutomationList = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useIgAutomationQuery(queryPage);
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row?.original?.trigger_type}</Text>
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row?.original?.dm.format}</Text>
          </div>
        );
      },
    },
        ...
        queryPage.page == 1 ? (
          <>
            ...
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              ...
              <Text size="2xl" weight="semibold">{`No Automation Lists`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

...
import AddNotificationSheet from "./AddNotificationSheet";
import EditNotificationSheet from "./EditNotificationSheet";
import { usePartnerNotificationQuery } from "@/framework/partner/workspace/notification/get-notification";
...
export const NotificationList = (props: Props) => {
  ...
  const { data, isLoading, refetch } = usePartnerNotificationQuery(queryPage);
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{`${
              row?.original?.body ? row?.original?.body : "----"
            }`}</Text>
          </div>
        );
      },
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="flex items-center gap-4">
            <EditNotificationSheet data={notification}>
              ...
              <Alert
                ...
                onRightButton={async () => {
                  ...
                  try {
                    await mutateAsync({
                      method: "DELETE",
                      notification_id: row?.original?._id,
                    });

...
import AddWhatsappSheet from "./AddWhatsappSheet";
import { PlusIcon } from "@/components/ui/icons/PlusIcon";
import { Button } from "@/components/ui/button";
import { usePartnerWorkspaceById } from "@/framework/partner/workspace/whatsapp/get-channel";

type Props = {};

export const WhatsappList = (props: Props) => {
  ...
  const { data, isLoading, refetch } = usePartnerWorkspaceById(queryPage);
      ...
      cell: ({ row }: any) => (
        <div className="flex items-center gap-3">
          <EyeIcon
            ...
            onClick={() => {
              router.push(`/app/partner/${partnerId}/workspace/${workspaceId}/workspace-overview/whatsapp/business/${row?.original?._id}/overview`);
            }}
          />
        </div>
      ),
        ...
        queryPage.page == 1 ? (
          <>
            ...
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              ...
              <Text size="2xl" weight="semibold">{`No Channel Lists`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

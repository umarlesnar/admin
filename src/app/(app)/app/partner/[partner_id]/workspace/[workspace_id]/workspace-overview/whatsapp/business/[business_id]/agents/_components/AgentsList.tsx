...
const AgentsList = () => {
  ...
  const { data, isLoading, refetch } = useBusinessUserByIdQuery(queryPage);

  useEffect(() => {
    if (data) {
      const filteredItems = data?.items?.filter(
        (agent: any) => agent.status !== "DELETED"
      );
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>
              {row?.original?.phone?.[0]?.mobile_number
                ? row?.original?.phone?.[0]?.mobile_number
                : "-"}
            </Text>
          </div>
        ...
        queryPage.page == 1 ? (
          <>
            ...
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              ...
              <Text size="2xl" weight="semibold">{`No Agents Lists`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

...
export const MessengerList = (props: Props) => {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row.getValue("phone_number")}</Text>
          </div>
        );
      },
    },
        ...
        queryPage.page == 1 ? (
          <>
            ...
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              <MessengerIcon className="w-12 h-12 text-primary" />
              <Text size="2xl" weight="semibold">{`No Messenger Lists`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

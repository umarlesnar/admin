...
import { useSystemRolesQuery } from "@/framework/iam/system-roles/get-system-roles";
import { EditIcon } from "@/components/ui/icons/EditIcone";
import AddUserRolesSheet from "./AddUserRoleSheet";
import EditUserRolesSheet from "./EditUserRoleSheet";
...
export const SystemRolesLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useSystemRolesQuery(queryPage);
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="flex items-center gap-4">
            <EditUserRolesSheet data={UserRole}>
              <EditIcon className="w-5 h-5 text-green-500 cursor-pointer" />
            </EditUserRolesSheet>
            <>
              <Alert
                icon={<AlertIcon className="w-[50px] h-[50px] text-red-500" />}
                description={`Do you want to remove this ${row?.original?.name} role ?`}
                onRightButton={async () => {
                  const loadingToast = toast.loading("Loading...");
                  try {
                    await mutateAsync({
                      method: "DELETE",
                      system_roles_id: row?.original?._id,
                    });
        ...
        queryPage.page == 1 ? (
          <>
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              ...
              <Text size="2xl" weight="semibold">{`No System Role Found`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

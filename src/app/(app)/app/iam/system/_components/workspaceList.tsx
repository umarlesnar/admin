...
import { useSystemWorkspaceQuery } from "@/framework/iam/system/get-workspace";
...
export const WorkspaceLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useSystemWorkspaceQuery(queryPage);
        ...
        queryPage.page == 1 ? (
          <>
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              ...
              <Text size="2xl" weight="semibold">{`No System Found`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

...
import { useClassicApiKeysQuery } from "@/framework/iam/api-key-management/get-classic-api-key";
...
export const ClassicLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useClassicApiKeysQuery(queryPage);
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row?.original?.api_key_type || "-"}</Text>
          </div>
        );
      },
    },
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text
              className={`${
                row?.original?.rate_limit?.window
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row?.original?.user_type || "-"}</Text>
          </div>
        );
      },
    },
        ...
        queryPage.page == 1 ? (
          <>
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              <ApiKeyIcon className="w-12 h-12 text-primary" />
              <Text size="2xl" weight="semibold">{`No Api Key Found`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable

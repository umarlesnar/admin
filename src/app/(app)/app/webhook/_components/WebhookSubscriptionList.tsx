...
export const WebhookSubscriptionLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useWebhookSubscriptionQuery(queryPage);
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="flex items-center gap-4">
            <EditWebhookSubscriptionSheet data={webhook}>
              <EditIcon className="w-4 h-4 cursor-pointer text-green-600" />
            </EditWebhookSubscriptionSheet>
            <>
              <Alert
                ...
                onRightButton={async () => {
                  ...
                  try {
                    await mutateAsync({
                      method: "DELETE",
                      webhook_subscription_id: row?.original?._id,
                    });

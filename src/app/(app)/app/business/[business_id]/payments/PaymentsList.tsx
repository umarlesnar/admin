...
// import { useBusinessTemplateByIdQuery } from "@/framework/business/get-business-template";
...
import { useBusinessPaymentByIdQuery } from "@/framework/business/get-business-payment";
import { Currency2Icon } from "@/components/ui/icons/Currency2Icon";

type Props = {};

const BusinessPaymentList = () => {
  ...
  const { data, isLoading, refetch } = useBusinessPaymentByIdQuery(queryPage);
  ...
  const columns: ColumnDef<any>[] = [
    {
      accessorKey: "plan", // Using accessorKey for first name
      header: "Plan",
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
            <Text>{row.getValue("plan")}</Text>
          </div>
        );
      },
    },
    {
      accessorKey: "type", // Using accessorKey for first name
      header: "Type",
      cell: ({ row }: any) => {
        return (
          <div className="flex items-center gap-2 font-medium text-text-secondary">
      ...
      cell: ({ row }: any) => (
        <div className="flex items-center gap-3">
          <EyeIcon
            ...
            onClick={() => {
              router.push(`/app/user-account/${row?.original?._id}/view`);
            }}
          />
        </div>
      ),
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="w-[40%] flex items-center gap-3">
          <div className="w-full">
            <SearchBox
              className="w-full"
              placeholder="Search payments by plan"
              onChange={(e) => {
        ...
        queryPage.page == 1 ? (
          <>
            ...
            <div className="w-full h-full flex items-center justify-center flex-col space-y-3 bg-white">
              <Currency2Icon className="w-16 h-16 text-primary" />
              <Text size="2xl" weight="semibold">{`No Payments Lists`}</Text>
            </div>
          </>
        ) : (
          <AsyncTable
...
export default BusinessPaymentList;

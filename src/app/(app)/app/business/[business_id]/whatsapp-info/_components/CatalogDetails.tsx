...
import { useCatalogProductSyncMutation } from "@/framework/business/catalog/catalog-product-sync-mutation";
import { useCatalogSetSyncMutation } from "@/framework/business/catalog/catalog-set-sync-mutation";
...
const CatalogDetails = (props: Props) => {
  ...
  const { mutateAsync, isPending } = useCatalogProductSyncMutation();
  const setSyncMutation = useCatalogSetSyncMutation();
  ...
  return (
    <Card className="w-full flex-1">
      ...
      <div className="px-6 py-1 flex justify-between items-center">
        ...
        <Button
          className=" text-[#304742] "
          variant="outline"
          leftIcon={
            <RefreshIcon
              className={`w-4 h-4 mr-1 ${isPending ? "animate-spin" : ""}`}
            />
          }
          onClick={async () => {
            try {
              await mutateAsync();
              await setSyncMutation?.mutateAsync();
            } catch (error) {
              console.log("error", error);
            }
          }}

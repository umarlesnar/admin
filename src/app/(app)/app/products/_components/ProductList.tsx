...
import AddProductSheet from "./AddProductSheets";
...
export const ProductLists = (props: Props) => {
  ...
  const { data, isLoading, refetch } = useProductsQuery(queryPage);
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
        ...
        cell: ({ row }: any) => {
          return (
            <div className="flex items-center gap-2 font-medium text-text-secondary">
              <Text>{`${row?.original?.currency_code}`}</Text>
            </div>
          );
        },
      },
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <div className="flex items-center gap-4">
            <EditProductSheet data={product}>
              <EditIcon className="w-4 h-4 cursor-pointer text-green-600" />
            </EditProductSheet>
            <>
              <Alert
                icon={<AlertIcon className="w-[50px] h-[50px] text-red-500" />}
                description={`Do you want to remove this ${row?.original?.name} Product ?`}
                onRightButton={async () => {
                  const loadingToast = toast.loading("Loading...");
                  try {
                    await mutateAsync({
                      method: "DELETE",
                      product_id: row?.original?._id,
                    });

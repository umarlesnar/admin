...
const WorkflowCustomerTypeSheet = ({ children, sheetData, id }: Props) => {
  const [open, setOpen] = useState<boolean>(false);
  const { updateNodeData } = useWorkflowStore();
  // const { data } = useAvailableAttributeQuery();
  const [selectedAttributes, setSelectedAttributes] = useState<any[]>([]);

  const MODE = [{ name: "upset", value: "upset" }];
  ...
  const getInitialValues = () => {
    let initial = {
      mode: sheetData?.mode || "upset",
      wa_id: sheetData?.wa_id || "",
      name: sheetData?.name || "",
      custom_params: [{ key: "", value: "", type: "" }],
    };

    if (sheetData?.custom_params) {
      if (Array.isArray(sheetData.custom_params)) {
        initial.custom_params = sheetData.custom_params.map(
          (param: CustomParam) => ({
            key: param.key || "",
            value: param.value || ""
          })
        ...
        typeof sheetData.custom_params === "object" &&
        sheetData.custom_params.key
      ) {
        initial.custom_params = [
          {
            key: sheetData.custom_params.key || "",
            value: sheetData.custom_params.value || "",
            type: sheetData.custom_params.type || "",
          },
        ];
      }
    }
  ...
  //       return (
  //         data.find(
  //           (item: any) => item.value === param.key || item.name === param.key
  //         ) || null
  //       );
  //     });
  ...
  const removeField = (index: number, values: any, setFieldValue: any) => {
    ...
    const newParams = values.custom_params.filter(
  ...
  const hasValidData = (values: any) => {
    return values.custom_params.some(
      (param: CustomParam) => param.key && param.value
    );
  };
  ...
  return (
    <Formik
      ...
      validate={(values) => {
        ...
        if (!values.wa_id)
          errors.wa_id = "Phone number is required include country code";
      ...
      onSubmit={(values) => {
        const filteredParams = values.custom_params.filter(
          (param: CustomParam) => param.key && param.value
        );
            ...
            <SheetContent className="w-[390px] sm:w-[450px] h-screen flex flex-col p-5">
              ...
              <div className="flex flex-1 flex-col px-1 py-6 overflow-auto bg-scroll space-y-4">
                ...
                <Combobox
                  ...
                  value={values.mode}
                  selectedOption={
                    MODE.find((item: any) => item.value === values.mode) || null
                  }
                  onSelectData={(item: any) =>
                    setFieldValue("mode", item.value)
                  }
                ...
                {values.custom_params.map(
                  (param: CustomParam, index: number) => (
                    <div
                      key={index}
                    ...
                    >
                      ...
                      {values.custom_params.length > 1 && (
                        <Button
                          ...
                          onClick={() =>
                            removeField(index, values, setFieldValue)
                          }
                        >
...
export default WorkflowCustomerTypeSheet;

...
import {
  yupTemplateLibrarySchema,
  yupTemplateSortQuery,
} from "@/validation-schema/api/yup-template-library-schema";
...
router
  ...
  .get(async (req: AppNextApiRequest, { params }: any) => {
    ...
    let templateLibraryQueryValidation: any = {};
    try {
      templateLibraryQueryValidation = yupFilterQuerySchema.validateSync(query);
    } catch (error) {
    ...
    if (templateLibraryQueryValidation.q != "") {
      searchQuery = {
        $or: [
          {
            name: {
              $regex: `.*${templateLibraryQueryValidation.q}.*`,
              $options: "i",
            },
          },
          {
    ...
    if (templateLibraryQueryValidation.filter !== "") {
      const originalFilterObj = getJSONObjectFromString(
        templateLibraryQueryValidation.filter
      );
    ...
    if (templateLibraryQueryValidation.sort != "") {
      const _lstry: any = templateLibraryQueryValidation?.sort;
      const sortObj = getJSONObjectFromString(_lstry);

      try {
        sortQuery = yupTemplateSortQuery.validateSync(sortObj);
      } catch (error) {
    ...
    const finalResponse: any = {
      per_page: templateLibraryQueryValidation.per_page,
      total_page: 0,
      total_result: 0,
      items: [],
      current_page: templateLibraryQueryValidation.page,
    };

    const totalCount = await templateLibraryModelSchema.countDocuments(
      finalFilterQuery
    );
    ...
    let skip: any =
      templateLibraryQueryValidation.page > 0
        ? (templateLibraryQueryValidation.page - 1) *
          templateLibraryQueryValidation.per_page
        : 0;
    let templates = await templateLibraryModelSchema
  ...
  .post(async (req: AppNextApiRequest, { params }: any) => {
    ...
    try {
      ...
      const newtemplate = new templateLibraryModelSchema({

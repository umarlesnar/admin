...
import userLoginActivityModelSchema from "@/models/user-login-activity-model-schema";
...
router
  ...
  .get(async (req: AppNextApiRequest, { params }: any) => {
    ...
    let UserLoginActivityQueryValidation: any = {};
    try {
      UserLoginActivityQueryValidation =
    ...
    let filterQuery: any = {};
    if (UserLoginActivityQueryValidation.filter) {
      const originalFilterObj = getJSONObjectFromString(
        UserLoginActivityQueryValidation.filter
      );
    ...
    let userLoginActivity = await userLoginActivityModelSchema

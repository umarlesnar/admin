...
import { yupBusinessCloudProviderSchema } from "@/validation-schema/api/yup-business-update-schema";
...
router
  ...
  .put(async (req: AppNextApiRequest, { params }: any) => {
    ...
    try {
      filterPayload = yupBusinessCloudProviderSchema.validateSync(body);
    } catch (error) {
    ...
    try {
      let cloud_provider = await businessAccountModelSchema.findOne({

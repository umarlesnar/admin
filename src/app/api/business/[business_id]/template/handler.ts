...
import { yupTemplateSortQuery } from "@/validation-schema/api/yup-template-library-schema";
  ...
  .get(async (req: AppNextApiRequest, { params }: any) => {
    ...
    let skip: any =
      BusinessTemplateQueryValidation.page > 0
        ...
        : 0;
    let businesstemplates = await businessTemplateModelSchema
      .find(finalFilterQuery)
      .skip(skip)
      .sort(sortQuery)
      .limit(finalResponse.per_page);
    finalResponse.items = businesstemplates;
    return NextResponse.json({
      data: finalResponse,
    });
  });

...
import { YupworkspaceOverviewSchema, YupworkspaceSchema } from "@/validation-schema/api/yup-workspace-schema";
      ...
      .put(async (req: AppNextApiRequest, { params }: any) => {
        ...
        try {
          const alreadyExist = await workspaceModelSchema.findOne({
            _id: { $ne: workspace_id },
            email_id: WorkspaceValidateBody.billing_address.email_id,
          });
            ...
            return NextResponse.json(
              {
                ...
                data: {
                 workspace : "Workspace Already Exist",
                },
              },
              {
                status: SERVER_STATUS_CODE.VALIDATION_ERROR_CODE,

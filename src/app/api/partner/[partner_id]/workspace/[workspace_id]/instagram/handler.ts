...
router
  ...
  .get(async (req: AppNextApiRequest, { params }: any) => {
    ...
    let InstagramQueryValidation: any = {};
    try {
        InstagramQueryValidation = yupFilterQuerySchema.validateSync(query);
    } catch (error) {
    ...
    try {
      ...
      if (InstagramQueryValidation.q != "") {
        searchQuery = {
          $or: [
            {
              name: {
                $regex: `.*${InstagramQueryValidation.q}.*`,
                $options: "i",
              },
            },
          ],
      ...
      if (InstagramQueryValidation.filter !== "") {
        const originalFilterObj = getJSONObjectFromString(
            InstagramQueryValidation.filter
        );
      ...
      const finalResponse: any = {
        per_page: InstagramQueryValidation.per_page,
        total_page: 0,
        total_result: 0,
        items: [],
        current_page: InstagramQueryValidation.page,
      };
      ...
      let skip: any =
      InstagramQueryValidation.page > 0
          ? (InstagramQueryValidation.page - 1) *
          InstagramQueryValidation.per_page
          : 0;
      let instagram = await businessAccountModelSchema
        .find(finalFilterQuery)
        .skip(skip)
        .sort(sortQuery)
        .limit(finalResponse.per_page);

      finalResponse.items = instagram;

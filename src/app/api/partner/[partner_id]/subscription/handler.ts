...
router
  ...
  .get(async (req: AppNextApiRequest, { params }: any) => {
    ...
    try {
      ...
      let searchQuery: any = {};
      if (
        subscriptionQueryValidation.q &&
        subscriptionQueryValidation.q.trim() !== ""
      ) {
        const escapedQuery = escapeRegex(subscriptionQueryValidation.q.trim());
        searchQuery = {
          $or: [
            {
              plan_name: {
      ...
      if (subscriptionQueryValidation.filter !== "") {
        ...
        if (subscription_status) {
          ...
          if (subscription_status === "active") {
            filterQuery.r_current_end_at = { $gt: currentTimestamp };
          } else if (subscription_status === "expired") {
            filterQuery.r_current_end_at = { $lte: currentTimestamp };
        ...
        if (start || end) {
          filterQuery.r_current_end_at = {};
          if (start) {
            filterQuery.r_current_end_at.$gte = Number(start);
          }
          if (end) {
            filterQuery.r_current_end_at.$lte = Number(end);
          }
        }
      ...
      // Build sort query
      let sortQuery: Record<string, 1 | -1> = { r_current_end_at: -1 };
      const sortParam = subscriptionQueryValidation.sort?.trim();
      if (sortParam) {
        const parsedSort = getJSONObjectFromString(sortParam);
        if (parsedSort?.r_current_end_at && !isNaN(Number(parsedSort.r_current_end_at))) {
          const direction = Number(parsedSort.r_current_end_at) === 1 ? 1 : -1;
          sortQuery = { r_current_end_at: direction };
        }
      }
      ...
      const aggregationPipeline = [
        ...
        // Add pagination metadata
        {
          $facet: {
            data: [
              { $skip: skip },
              { $limit: subscriptionQueryValidation.per_page },
            ],
      ...
      const [result] = await subscriptionSchema.aggregate(aggregationPipeline);
      ...
      const finalResponse = {
        ...
        total_page: Math.ceil(
          totalCount / subscriptionQueryValidation.per_page
        ),

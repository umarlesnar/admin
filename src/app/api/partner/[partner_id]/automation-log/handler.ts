...
import igAutomationSessionSchema from "@/models/ig-automation-session-schema";
...
router
  .use(apiMiddlerware)
    .get(async (req: AppNextApiRequest, { params }: any) => {
      ...
      let igAutomationLogQueryValidation: any = {};
  
      try {
        igAutomationLogQueryValidation = yupFilterQuerySchema.validateSync(query);
      } catch (error) {
      ...
      try {
        ...
        let searchQuery: any = {};
        if (
          igAutomationLogQueryValidation.q &&
          igAutomationLogQueryValidation.q.trim() !== ""
        ) {
          const escapedQuery = escapeRegex(igAutomationLogQueryValidation.q.trim());
          searchQuery = {
            $or: [
              {
                type: {
        ...
        // Build filter query
        let filterQuery: any = {};
        if (igAutomationLogQueryValidation.filter !== "") {
          const originalFilterObj = getJSONObjectFromString(
            igAutomationLogQueryValidation.filter
          );
        ...
        const sortParam = igAutomationLogQueryValidation.sort?.trim();
        if (sortParam) {
        ...
        const skip =
        igAutomationLogQueryValidation.page > 0
            ? (igAutomationLogQueryValidation.page - 1) *
            igAutomationLogQueryValidation.per_page
            : 0;
        ...
        const aggregationPipeline = [
          ...
          // Add pagination metadata
          {
            $facet: {
              data: [
                { $skip: skip },
                { $limit: igAutomationLogQueryValidation.per_page },
              ],
        ...
        const [result] = await igAutomationSessionSchema.aggregate(
          aggregationPipeline,
          { allowDiskUse: true }
        );
  
        const totalCount = result.totalCount[0]?.count || 0;
        const igAutomationLog = result.data || [];
  
        const finalResponse = {
          per_page: igAutomationLogQueryValidation.per_page,
          total_page: Math.ceil(totalCount / igAutomationLogQueryValidation.per_page),
          total_result: totalCount,
          items: igAutomationLog,
          current_page: igAutomationLogQueryValidation.page,
        };
